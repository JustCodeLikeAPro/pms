...
/**
 * Login screen
 * - Branding: Ava logo + "Trinity PMS"
 * - Taglines & AI message
 * - Select Language + Mode (Light/Dark)
 * - Static OTP (000000) for now
 * - Pre-registered users notice + Terms/Privacy links
 */
export default function Login() {
  ...
  // Apply "dark" mode by toggling classes on <html>. We don't rely on Tailwind dark: variants.
  useEffect(() => {
    const root = document.documentElement;
    if (dark) root.classList.add('dark');
    else root.classList.remove('dark');
    // We conditionally render tailwind classes below, so this is mostly cosmetic for consistency.
  }, [dark]);
  ...
  // Helper: shared card styles that respect our manual dark toggle
  ...
  return (
    <div
      className={
        // Green–yellow hero gradient; dark flips to neutral
        dark
          ? 'min-h-screen flex items-stretch bg-gray-950'
          : 'min-h-screen flex items-stretch bg-gradient-to-br from-lime-100 via-yellow-50 to-emerald-100'
      }
    >
      {/* Left brand/hero panel */}
      <section className="hidden lg:flex w-1/2 p-10">
      ...
      {/* Right auth card */}
      <section className="flex w-full lg:w-1/2 items-center justify-center p-6 sm:p-10">
        <div className={`w-full max-w-md rounded-2xl border shadow-sm p-6 sm:p-8 space-y-6 ${card}`}>
          {/* Top controls: language + mode */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={`text-sm ${subtle}`}>Language</span>
              <select
          ...
          <div className="lg:hidden flex items-center gap-3">
            <AvaLogo />
            <div>
              <div className="text-xl font-bold tracking-tight">Trinity PMS</div>
              <div className={`text-xs ${subtle}`}>Empowering Projects</div>
            </div>
          </div>
          ...
          {step === 'enter' && (
            <div className="space-y-4">
              ...
              <div className="text-xs leading-relaxed">
                ...
                <p className="text-[11px] mt-2">Dev OTP: <b>000000</b></p>
              </div>
            </div>
          )}
          ...
          {step === 'otp' && (
            <div className="space-y-4">
              ...
              <div className="text-xs">
                <p className={subtle}>Didn’t get it? For the demo, OTP is <b>000000</b>.</p>
              </div>
            </div>
          )}
...
/** Simple inline logo to avoid assets; green/yellow palette */
function AvaLogo() {
  return (
    <div
      aria-label="Ava Logo"
