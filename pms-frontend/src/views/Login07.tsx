 ...
 * views/Login.tsx
 * ---------------
 * - On "Send OTP" (or Enter), we VALIDATE USERNAME via /auth/exists?login=...
 *   • If exists: go to OTP step (we DO NOT call /auth/otp/request here).
 *   • If not: show "User does not exist. Check if the username is correct!"
 * - OTP verify stays the same (POST /auth/otp/verify).
 * - Super Admin -> /admin, others -> /landing.
 * - Remember me + suggestions + manage saved logins modal preserved.
 */
...
const SAVED_LOGINS_KEY = 'savedLogins'; // JSON string array
...
function addSavedLogin(login: string) {
  ...
  writeSavedLogins(current.slice(0, 8)); // keep last 8
}
function removeSavedLogin(login: string) {
  const current = readSavedLogins();
  const next = current.filter((v) => v.toLowerCase() !== (login || '').toLowerCase());
...
function forceNavigate(path: string) {
  ...
  // If something (guard/interceptor) blocks it, hard-redirect as fallback
  setTimeout(() => {
    if (window.location.pathname !== path) {
      console.log('[Login] router nav did not stick. Hard redirect →', path);
...
export default function Login() {
  // Prefill with most recent remembered, if any
  const initialLogin = (() => {
  ...
  // Remember-me checkbox (unchecked does not erase existing list)
  ...
  const validateUser = async () => {
    ...
    try {
      setBusy(true);
      // Expecting backend endpoint: GET /auth/exists?login=...
      // Response shape example: { ok: true, exists: boolean }
      const { data } = await api.get('/auth/exists', { params: { login } });
      if (!data?.ok) {
        // If backend returns ok:false, treat as not found
        setErr('User does not exist. Check if the username is correct!');
        return;
      }
      if (data.exists === true) {
        console.log('[Login.validateUser] user exists → showing OTP');
        setStep('otp'); // proceed to OTP input; we did NOT trigger OTP send here
      } else {
        setErr('User does not exist. Check if the username is correct!');
      }
    } catch (e: any) {
      // If endpoint not implemented yet or any network error:
      const msg =
...
const verify = async () => {
  ...
  try {
    setBusy(true);
    console.log('[Login.verify] submitting OTP verify for', login);
    ...
    if (!data?.ok) {
      ...
      console.log('[Login.verify] not ok →', msg);
      setErr(msg);
      return;
    }
    ...
    // Add to saved list only if remember is checked
    if (remember) {
      addSavedLogin(login);
      setSavedLogins(readSavedLogins());
      console.log('[Login.verify] remember=true → saved login');
    }

    // Determine admin from JWT payload first, fallback to user flag
...
//     // Add to saved list only if remember is checked (do not erase otherwise)
//     if (remember) {
//       addSavedLogin(login);
//       setSavedLogins(readSavedLogins());
//     }

//     // Determine admin from JWT payload first
//     const payload = decodeJwtPayload(data.token);
//     const isAdmin = !!(payload && payload.isSuperAdmin);
//     console.debug('[Login.verify] payload:', payload, 'user.isSuperAdmin:', data.user?.isSuperAdmin);

//     if (isAdmin || data.user?.isSuperAdmin) {
//       console.debug('[Login.verify] admin detected → navigating to /admin');
//       // tiny defer prevents any synchronous state update from interfering
//       setTimeout(() => nav('/admin', { replace: true }), 0);
//       return;
//     }

//     console.debug('[Login.verify] non-admin → /landing');
//     setTimeout(() => nav('/landing', { replace: true }), 0);
//   } catch (e: any) {
...
// };
  ...
  const onEnterLogin = (e: React.KeyboardEvent<HTMLInputElement>) => e.key === 'Enter' && validateUser();
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-6 space-y-4">
            ...
            {step === 'enter' && (
              <>
                ...
                <button onClick={validateUser} className={btnPrimary} disabled={busy || !login.trim()}>
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    title="View or remove remembered usernames"
                  >
                    Manage saved logins
                  </button>
                </div>
